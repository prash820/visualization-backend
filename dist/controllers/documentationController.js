...
const generateHighLevelDocumentation = async (req, res) => {
    try {
...
const generateLowLevelDocumentation = async (req, res) => {
    try {
        const { prompt, umlDiagrams } = req.body;
        if (!prompt) {
            res.status(400).json({ error: 'Prompt is required' });
...
async function generateComprehensiveDocumentation(prompt, umlDiagrams) {
...
async function generateHighLevelDesign(prompt, umlDiagrams) {
...
async function generateLowLevelDesign(prompt, umlDiagrams) {
